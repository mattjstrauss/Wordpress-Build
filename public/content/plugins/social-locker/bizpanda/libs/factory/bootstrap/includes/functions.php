...
class FactoryBootstrap330_Manager {
    ...
    public function loadAssets( $hook ) {
        ...
        if ( !$compression ) {
            ...
            foreach($this->scripts as $scriptToLoad) {
                wp_enqueue_script($scriptToLoad . '-' . $id, FACTORY_BOOTSTRAP_330_URL . "/assets/js/$scriptToLoad.js", $isFirst ? $dependencies : false);
                $isFirst = false;            
            }

            foreach($this->styles as $styleToLoad) {
                wp_enqueue_style($styleToLoad . '-' . $id, FACTORY_BOOTSTRAP_330_URL . "/assets/flat/css/$styleToLoad.css" );       
            }
        ...
        // - //
            ...
            if ( !empty( $this->styles ) ) {
                ...
                wp_enqueue_style('factory-bootstrap-330-' . $id, FACTORY_BOOTSTRAP_330_URL . '/includes/load-styles.php?c=1&folder=flat&load='.$loadStylesOut, array(), FACTORY_BOOTSTRAP_330_VERSION); 
            }

            if ( !empty( $this->scripts ) ) {
                $id = md5($loadScriptsOut . FACTORY_BOOTSTRAP_330_VERSION);
                wp_enqueue_script('factory-bootstrap-330-' . $id, FACTORY_BOOTSTRAP_330_URL . '/includes/load-scripts.php?c=1&load='.$loadScriptsOut, $dependencies, FACTORY_BOOTSTRAP_330_VERSION); 
            } 
        ...
        <script>
            ...
            if ( !window.factory.factoryBootstrap330 ) window.factory.factoryBootstrap330 = {}; 
    ...
    public function testKeepingArgsInPaths( $src, $handle ) {
        if ( substr($handle, 0, 22) !== 'factory-bootstrap-330-') return $src;
